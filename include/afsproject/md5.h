...
// NOLINTNEXTLINE
const std::vector<uint32_t> SHIFT_AMOUNTS = { 7,
  12,
  17,
  22,
  7,
...
public:
  ...
  void finalize();
  void step(std::vector<uint32_t> &);

  static std::string to_hex_string(const std::vector<uint8_t> &);
  static std::vector<uint8_t> to_byte_vector(const std ::string &);

  std::vector<uint8_t> string(const std::string &);

private:
  std::unique_ptr<MD5Context> m_ctx;
};
